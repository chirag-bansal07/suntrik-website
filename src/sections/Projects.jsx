...
const projects = [
  {
    id: 1, title: 'Commercial Rooftop Installation',
    location: 'Surat, Gujarat', capacity: '250 kWp',
    type: 'Commercial', year: '2024',
    desc: 'Grid-tied rooftop solar system for a textile manufacturing unit, reducing energy bills by 70%.',
    img: null,
  },
  {
    id: 2, title: 'Industrial Ground Mount',
    location: 'Pune, Maharashtra', capacity: '1.2 MWp',
    type: 'Industrial', year: '2023',
    desc: 'Large-scale ground-mounted plant supplying clean power to a pharmaceutical facility.',
    img: null,
  },
  {
    id: 3, title: 'Residential Solar Project',
    location: 'Ahmedabad, Gujarat', capacity: '10 kWp',
    type: 'Residential', year: '2024',
    desc: 'Premium mono PERC rooftop system with battery backup for uninterrupted power supply.',
    img: null,
  },
  {
    id: 4, title: 'Government Solar Farm',
    location: 'Rajasthan', capacity: '5 MWp',
    type: 'Utility', year: '2023',
    desc: 'Utility-scale solar farm under government renewable energy program, commissioned on schedule.',
    img: null,
  },
]
    ...
    <section id="projects" ref={containerRef} style={{ background: 'var(--bg-elevated)', padding: '7rem 0', overflow: 'hidden' }}>
      <div className="container">
        <div ref={ref} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '3.5rem', flexWrap: 'wrap', gap: '1.5rem' }}>
          <div>
            ...
            >
              Proven Track <span className="gradient-text">Record</span>
            </motion.h2>
          </div>
          <motion.p
            initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
            style={{ color: 'var(--text-secondary)', maxWidth: 360, lineHeight: 1.75 }}
          >
            A portfolio of successful solar installations across India — from rooftops to utility-scale farms.
          </motion.p>
        </div>

        {/* Staggered grid with parallax pairs */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          ...
          <motion.div style={{ y: y2, display: 'flex', flexDirection: 'column', gap: '1.5rem', marginTop: '3rem' }}>
            {projects.filter((_, i) => i % 2 !== 0).map((p, i) => (
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          #projects .container > div:nth-child(2) > div:last-child { margin-top: 0 !important; }
        }
      `}</style>
    </section>
  )
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {p.img
          ...
          : <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.75rem' }}>Project photo</p>
            </div>
        }
        {/* Type badge */}
        <span style={{
      ...
      <div style={{ padding: '1.25rem' }}>
        ...
        <p style={{ fontSize: '0.78rem', color: 'var(--brand-orange)', fontWeight: 600, marginBottom: '0.6rem' }}>📍 {p.location} · {p.year}</p>
